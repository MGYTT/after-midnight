...
export default function AdminPage() {
      ...
      <main className="min-h-screen flex items-center justify-center px-6">
        <div className="w-full max-w-xs">
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && adminPass === process.env.NEXT_PUBLIC_ADMIN_PASS) {
                setAuthed(true)
              }
            }}
  ...
  const inputClass = "w-full bg-transparent border-b border-[#1e1e1e] pb-3 pt-1 text-[#e8e0d4] text-sm tracking-wide placeholder:text-[#2a2520] focus:outline-none focus:border-[#c9a84c]/50 transition-colors duration-500"
  ...
  return (
    <main className="min-h-screen px-6 py-16 max-w-md mx-auto">
      ...
      <div className="space-y-10">
        ...
        <div>
          <label className="text-[10px] tracking-[0.3em] text-[#3a3530] uppercase block mb-3">główna treść</label>
        ...
        <div>
          ...
          <input className={inputClass} value={form.audio_url}
        ...
        <motion.button
          onClick={handleSave}
          disabled={!form.title || !form.main_text || saving}
          whileTap={{ scale: 0.97 }}
          className="w-full py-4 text-[11px] tracking-[0.35em] uppercase text-[#c9a84c] border border-[#c9a84c]/20 hover:border-[#c9a84c]/50 transition-all duration-500 disabled:opacity-20 disabled:cursor-not-allowed"
        >
          <AnimatePresence mode="wait">
            ...
            >
              {saved ? 'zapisano ✓' : saving ? '...' : 'opublikuj'}
            </motion.span>
          </AnimatePresence>
        </motion.button>
      </div>
